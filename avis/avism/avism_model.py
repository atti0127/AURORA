...
from .modeling.aurora import (
    AVISM_TRACK_ADMISSION_THRESHOLD,
    aurora_score,
    refine_routed_masks,
    route_local_queries,
    ...
    def inference(self, batched_inputs):
        ...
        with torch.no_grad():
            for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
                ...
                outputs, chunk_frame_queries, chunk_mask_features = (
                    self.sem_seg_head(features, audio_features_chunk)
                )
                ...
                if self.aurora_enabled:
                    local_mask_logits.append(
                        outputs["pred_masks"].to(to_store)
                    )
        ...
        if self.aurora_enabled:
            ...
            if clip_embeddings is None or frame_embeddings is None:
                raise RuntimeError(
                    "AURORA requires AVISM similarity embeddings; "
        ...
        for i in range(
            math.ceil(
                len(mask_features) / self.test_interpolate_chunk_size
            )
        ):
            ...
            numerator += (
                interim_mask_soft.flatten(1)
                * interim_mask_hard.flatten(1)
            ).sum(1)
        ...
        indices = torch.nonzero(
            canonical_scores > AVISM_TRACK_ADMISSION_THRESHOLD
        ).squeeze(-1)
