...
AVISM_TRACK_ADMISSION_THRESHOLD = 0.3


def route_local_queries(clip_embeddings, frame_embeddings, query_indices):
    """Route selected video queries to frame queries.

    Args:
        clip_embeddings: Video-query embeddings with shape ``[Q, C]``.
        frame_embeddings: Frame-query embeddings with shape ``[T, F, C]``.
        query_indices: Selected video-query indices with shape ``[N]``.
    """
