...
class ACVIS_SAOC(nn.Module):
    ...
    def inference(self, batched_inputs):
        ...
        local_mask_logits = (
            torch.cat(local_mask_logits) if local_mask_logits else None
        )
        ...
        clip_embeddings = (
            acvis_outputs["pred_cq_embed"][-1, 0]
            if self.aurora_enabled else None
        )
        frame_embeddings = (
            acvis_outputs["pred_fq_embed"][-1, 0]
            if self.aurora_enabled else None
        )
        ...
        if self.aurora_enabled:
            local_query_indices, local_positive_matches = route_local_queries(
                clip_embeddings, frame_embeddings, topk_indices
            )
        else:
            local_query_indices, local_positive_matches = None, None
        ...
        if self.aurora_enabled:
            routed_totals = {
                name: torch.zeros(
                    len(mask_embed), dtype=torch.float, device=self.device
                )
                for name in (
                    "hard_intersection",
                    "global_area",
        ...
        for i in range(math.ceil(len(mask_features) / self.test_interpolate_chunk_size)):
            start = i * self.test_interpolate_chunk_size
                ...
                (i + 1) * self.test_interpolate_chunk_size, len(mask_features)
            )
            m_f = mask_features[start:end].to(self.device)

            mask_pred = torch.einsum("qc,tchw->qthw", mask_embed, m_f)

            if self.aurora_enabled:
                local_chunk = local_mask_logits[start:end].to(self.device)
                frame_index = torch.arange(
                    end - start, dtype=torch.long, device=self.device
                ).unsqueeze(0)
                chunk_query_indices = local_query_indices[:, start:end]
                matched_local_pred = local_chunk[
                    frame_index, chunk_query_indices
                ]
                positive = local_positive_matches[:, start:end]
                spatial_support = (
                    (mask_pred > 0.0) & (matched_local_pred > 0.0)
                ).flatten(2).any(dim=-1)

                chunk_statistics = routed_mask_statistics(
                    mask_pred, matched_local_pred, spatial_support
                )
                for name, values in chunk_statistics.items():
                    routed_totals[name] += values


                fused_pred = 0.5 * (mask_pred + matched_local_pred)
                mask_pred = torch.where(
                    positive[:, :, None, None], fused_pred, mask_pred
                ).masked_fill(
        ...
        canonical_scores = scores_per_video * (
            numerator / (denominator + 1e-6)
        )
        if self.aurora_enabled:
            hard_dice = 2.0 * routed_totals["hard_intersection"] / (
                routed_totals["global_area"]
                + routed_totals["local_area"]
                + 1e-6
            )
        ...
        confidence = 0.3
        indices = torch.nonzero(canonical_scores > confidence).squeeze(-1).cpu()
