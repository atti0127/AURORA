...
at::Tensor SMM_QmK_forward_cuda(const at::Tensor &A, const at::Tensor &B, const at::Tensor &index) {
    ...
    TORCH_CHECK(index.is_contiguous(), "Index tensor must be contiguous");
...
std::vector<at::Tensor> SMM_QmK_backward_cuda(const at::Tensor &grad_output,
    ...
    TORCH_CHECK(A.is_contiguous(), "A tensor has to be contiguous");
    TORCH_CHECK(B.is_contiguous(), "B tensor has to be contiguous");
    TORCH_CHECK(index.is_contiguous(), "index tensor has to be contiguous");
