...
def format_avis_metrics(metrics, dataset_name=None, gt_file=None):
    """Format official AVIS metrics without changing their computation."""
    ...
    if remaining:
        ...
        for key in remaining:
            ...
            if isinstance(value, (int, np.integer)):
                ...
                # H2S also returns per-sequence diagnostic arrays. They remain
                # available to the evaluator but do not belong in the concise
                # human-readable metric companion.
                continue
...
def eval_track(out_dir, gt_folder, gt_file):
    freeze_support()
...
class AVISEvaluator(DatasetEvaluator):
    def __init__(
        ...
        self._gt_folder = os.path.dirname(json_file)
        self._gt_file = os.path.basename(json_file)
    ...
    def _eval_predictions(self, predictions):
        ...
        assert o_d != None
        output_res = eval_track(o_d, self._gt_folder, self._gt_file)
        ...
        metrics_path = write_avis_metrics(
            ...
            gt_file=os.path.join(self._gt_folder, self._gt_file),
        )
