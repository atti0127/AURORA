...
from .modeling.aurora import (
    H2S_TRACK_ADMISSION_THRESHOLD,
    aurora_score,
    refine_routed_masks,
    route_local_queries,
    routed_dice_statistics,
)
...
class AVISM_COCO(nn.Module):
        ...
        *,
        ...
        if freeze_detector:
            for name, p in self.named_parameters():
                if "vita_module" not in name:
                    p.requires_grad_(False)
        self.test_run_chunk_size = test_run_chunk_size
        self.test_interpolate_chunk_size = test_interpolate_chunk_size
        self.aurora_enabled = aurora_enabled
        self.aurora_export_features = aurora_export_features
        if self.aurora_export_features and not self.aurora_enabled:
            raise ValueError("AURORA feature export requires AURORA_ENABLED")
    ...
    def from_config(cls, cfg):
        ...
        avism_module = Avism_COCO(
            cfg=cfg, in_channels=hidden_dim, aux_loss=avism_deep_supervision
        )
        ...
        avism_weight_dict = {
            "loss_avism_ce": class_weight,
            "loss_avism_mask": mask_weight,
            "loss_avism_dice": dice_weight,
        }
        if sim_weight > 0.0:
        ...
        if avism_deep_supervision:
            ...
            for i in range(avism_dec_layers - 1):
                aux_weight_dict.update(
                    {k + f"_{i}": v for k, v in avism_weight_dict.items()}
                )
        ...
        return {
            ...
            "aurora_enabled": cfg.MODEL.AVISM.AURORA_ENABLED,
            "aurora_export_features": cfg.MODEL.AVISM.AURORA_EXPORT_FEATURES,
    ...
    def train_model(self, batched_inputs):
        ...
        for video in batched_inputs:
            ...
            for audio_feat in video["audio"]:
                audio_features.append(torch.as_tensor(audio_feat).to(self.device))
        ...
        mask_features = mask_features.view(
            B, self.num_frames, *mask_features.shape[-3:]
        )
        ...
        avism_outputs["pred_masks"] = torch.einsum(
            "lbqc,btchw->lbqthw", avism_outputs["pred_mask_embed"], mask_features
        )
        for out in avism_outputs["aux_outputs"]:
            out["pred_masks"] = torch.einsum(
                "lbqc,btchw->lbqthw", out["pred_mask_embed"], mask_features
            )
        ...
        for k in list(losses.keys()):
            if k in self.criterion.weight_dict:
                ...
                losses.pop(k)
        avism_loss_dict = self.vita_criterion(
            avism_outputs, clip_targets, frame_targets, fg_indices
        )
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_video in targets:
            ...
            gt_classes_per_video = targets_per_video["instances"][0].gt_classes.to(
                self.device
            )
            gt_ids_per_video = []
            ...
            clip_gt_instances.append(
                {
                    ...
                    "video_len": targets_per_video["length"],
    ...
    def inference(self, batched_inputs):
        ...
        audio_features = torch.as_tensor(batched_inputs["audio"]).to(self.device)

        with torch.no_grad():
            for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
                start = i * self.test_run_chunk_size
                end = (i + 1) * self.test_run_chunk_size
                images = batched_inputs["image"][start:end]
                images = [
                    (x.to(self.device) - self.pixel_mean) / self.pixel_std
                    for x in images
                ]
                images = ImageList.from_tensors(images, self.size_divisibility)
                audio_features_chunk = audio_features[start:end]

                features = self.backbone(images.tensor)
                outputs, chunk_frame_queries, chunk_mask_features = self.sem_seg_head(
                    features, audio_features_chunk
                )
                chunk_mask_features = self.vita_module.vita_mask_features(
                    chunk_mask_features
                )

                frame_queries.append(chunk_frame_queries[-1])
                mask_features.append(chunk_mask_features.to(to_store))
                if self.aurora_enabled:
                    local_mask_logits.append(outputs["pred_masks"].to(to_store))
        ...
        if self.aurora_enabled:
            local_mask_logits = torch.cat(local_mask_logits)

        avism_outputs = self.vita_module(frame_queries, audio_features)
        mask_cls = avism_outputs["pred_logits"][-1, 0]
        mask_embed = avism_outputs["pred_mask_embed"][-1, 0]

        if self.aurora_enabled:
            clip_embeddings = avism_outputs["pred_cq_embed"][-1, 0]
            frame_embeddings = avism_outputs["pred_fq_embed"][-1, 0]
            if clip_embeddings is None or frame_embeddings is None:
                raise RuntimeError(
                    "AURORA requires H2S similarity embeddings; "
                    "MODEL.AVISM.SIM_WEIGHT must be positive"
                )
        ...
        class_scores_per_video = scores_per_video.clone()
        ...
        if self.aurora_enabled:
            route = route_local_queries(
                clip_embeddings, frame_embeddings, topk_indices,
            )
            local_query_indices = route["local_query_indices"]
            positive_matches = route["positive_matches"]

        mask_embed = mask_embed[topk_indices]
        track_count = len(mask_embed)
        numerator = torch.zeros(
            track_count, dtype=torch.float, device=self.device
        )
        denominator = torch.zeros_like(numerator)
        masks_per_video = []

        if self.aurora_enabled:
            hard_intersection = torch.zeros_like(numerator)
            global_area = torch.zeros_like(numerator)
            local_area = torch.zeros_like(numerator)

        for i in range(
            math.ceil(len(mask_features) / self.test_interpolate_chunk_size)
        ):
            start = i * self.test_interpolate_chunk_size
            end = min(
                (i + 1) * self.test_interpolate_chunk_size,
                len(mask_features),
            )
            chunk_mask_features = mask_features[start:end].to(self.device)
            mask_pred = torch.einsum(
                "qc,tchw->qthw", mask_embed, chunk_mask_features
            )
            ...
            if self.aurora_enabled:
                ...
                chunk_query_indices = local_query_indices[:, start:end]
                matched_local = local_mask_logits[start:end].to(self.device)[
                    frame_index, chunk_query_indices
                ]
                chunk_positive = positive_matches[:, start:end]

                refined, spatial_support = refine_routed_masks(
                    mask_pred, matched_local, chunk_positive
                )
                dice = routed_dice_statistics(
                    mask_pred, matched_local, spatial_support
                )
                hard_intersection += dice["hard_intersection"]
                global_area += dice["global_area"]
                local_area += dice["local_area"]
            ...
            numerator += (
                interim_mask_soft.flatten(1) * interim_mask_hard.flatten(1)
            ).sum(1)
            ...
            masks_per_video.append(output_masks.to(to_store))

        masks_per_video = torch.cat(masks_per_video, dim=1)
        pixel_quality = numerator / (denominator + 1e-6)
        canonical_scores = class_scores_per_video * pixel_quality

        if self.aurora_enabled:
            hard_dice = (
                2.0 * hard_intersection
                / (global_area + local_area + 1e-6)
            )
            scores_per_video = aurora_score(canonical_scores, hard_dice)
        else:
            scores_per_video = canonical_scores

        indices = torch.nonzero(
            canonical_scores > H2S_TRACK_ADMISSION_THRESHOLD
        ).squeeze(-1)
        scores_per_video = scores_per_video[indices]
        labels_per_video = labels_per_video[indices]
        mask_indices = (
            indices
            if masks_per_video.device == indices.device
            else indices.cpu()
        )
        masks_per_video = masks_per_video[mask_indices]
        ...
        if self.aurora_export_features:
            processed_results["pred_score_features"] = {
                "class_confidence": class_scores_per_video[indices].tolist(),
                "pixel_quality": pixel_quality[indices].tolist(),
                "hard_dice": hard_dice[indices].tolist(),
            }
